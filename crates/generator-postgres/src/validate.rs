...
fn validate_db<T: RenameExt>(v: &T, rn: &RenameMap) {
	...
	if str.len() > MAX_IDENTIFIER_LEN {
		panic!("{str} is larger than max allowed identifier! consider renaming")
	}
}
	...
	for ele in schema.items() {
		...
		match ele {
				...
				for ele in t.columns() {
					...
					for ele in &ele.annotations {
						match ele {
							...
							ColumnAnnotation::Default(d) => {}
						}
					}
				}
				for ele in t.annotations.iter() {
					match ele {
			...
			schema::SchemaItem::Enum(e) => {
				for ele in e.items.iter() {
					validate_db(ele, rn);
				}
			}
			schema::SchemaItem::Scalar(s) => {
				for ele in s.annotations.iter() {
					match ele {
