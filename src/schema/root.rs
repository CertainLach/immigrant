use crate::names::{DbTable, DbType, TableIdent, TypeIdent};
use crate::{newty_enum, EnumDiff, SchemaDiff, SchemaEnum, SchemaScalar, SchemaTable, TableDiff};
...
impl SchemaDiff<'_> {
	...
	fn created_enums(&self) -> Vec<SchemaEnum<'_>> {
	...
	fn dropped_enums(&self) -> Vec<SchemaEnum<'_>> {
		let old_enums = self.old.db_enums();
		let new_enums = self.new.db_enums();
	...
	fn altered_enums(&self) -> Vec<EnumDiff<'_>> {
		let old_enums = self.old.db_enums();
		let new_enums = self.new.db_enums();
	...
	pub fn print(&self, out: &mut String) {
		...
		for table in self.dropped_tables() {
			table.print_drop_foreign_keys(out);
		}
		for table_diff in self.altered_tables() {
			table_diff.print_renamed_or_fropped_foreign_keys(out);
		}

		for table_diff in self.altered_tables() {
			table_diff.print(out);
		}
		for table in self.dropped_tables() {
			table.drop(out);
		}
		...
		for table_diff in self.altered_tables() {
			table_diff.print_added_foreign_keys(out);
		}
		for table in self.created_tables() {
			table.create_fks(out);
		}
	}
}
